...
Test('/transaction', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         * responses: 200, 400
         */
        t.test('test getAllTransaction get operation', function (t) {
            Mockgen().requests({
                path: '/transaction',
                operation: 'get'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/transaction']['get']['responses']['200']['schema']);
         ...
         */
        t.test('test addTransaction post operation', function (t) {
            Mockgen().requests({
                path: '/transaction',
                operation: 'post'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/transaction']['post']['responses']['201']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
